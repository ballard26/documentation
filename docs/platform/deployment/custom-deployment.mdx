...
When `storage_strict_data_init` is set to `true` on a node, Redpanda will look for an empty file named `.redpanda_data_dir` in the root of its data volume. If the file is not there Redpanda will not start. This file must be created manually when the data volume is created. Its existence enables Redpanda to distinguish whether the data volume it's reading from is correct or not.
